...
km.perm <- KMeansSparseCluster.permute(df_matrix, K = 2:10, wbounds = seq(3, 7, len = 15), nperms = 5)
print(km.perm)
plot(km.perm)
optimal_k <- which.max(km.perm$gaps)  # Identify the optimal number of clusters
best_w <- km.perm$bestw  # Get the best tuning parameter
km.out <- KMeansSparseCluster(df_matrix, K = optimal_k, wbounds = best_w)
print(km.out)
plot(km.out)
df$cluster <- km.out[[1]]$Cs
df <- df[order(df$cluster),]
```
